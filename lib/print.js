exports.print = function (data) {
    if (data.errNum == 0) {
        ...
        var todayStr = " " + data.retData.city.bold + "天气 ".bold + today.type.yellow;
        ...
        today.index.forEach(function(item, index){
            console.log(" " + (++index) + "." + item.name + " " + item.index.yellow + " " + item.details.yellow);
            console.log();
        });
        ...
        forecast.forEach(function (data) {
            var str = " " + data.date + " " + data.type.yellow;
